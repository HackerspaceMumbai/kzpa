...
public class PlacesDatabase {
	...
	public static final String KEY_PLACE = SearchManager.SUGGEST_COLUMN_TEXT_1;
	public static final String KEY_COORDINATES = SearchManager.SUGGEST_COLUMN_TEXT_2;
	...
	private static HashMap<String, String> buildColumnMap() {
		...
		map.put(KEY_PLACE, KEY_PLACE);
		map.put(KEY_COORDINATES, KEY_COORDINATES);
		map.put(KEY_FUZZY, KEY_FUZZY);
	 ...
	 */
	public Cursor getPlace(String rowId, String[] columns) {
	...
	/**
	 * Returns a Cursor over all places that match the given query
	 * 
	 * @param query
	...
	public Cursor getWordMatches(String query, String[] columns) {
		...
		Log.d("The search query is ", query);
		String selection = KEY_FUZZY + " MATCH ?";
		...
		/*
		 * This builds a query that looks like: SELECT <columns> FROM <table>
		 * WHERE <KEY_PLACE> MATCH 'query*' which is an FTS3 search for the
		 * query text (plus a wildcard) inside the word column.
		 * 
		 ...
		 * clause to use FTS_VIRTUAL_TABLE instead of KEY_PLACE (to search
		 * across the entire table, but sorting the relevance could be
		 * difficult.
		 */
	}
	...
	private static class DictionaryOpenHelper extends SQLiteOpenHelper {
		...
		private static final String FTS_TABLE_CREATE = "CREATE VIRTUAL TABLE "
				+ FTS_VIRTUAL_TABLE + " USING fts3 (" + KEY_PLACE + ", "
				+ KEY_COORDINATES + ", " + KEY_FUZZY + ");";
		...
		private void loadPlaceName() throws IOException {
		 ...
		 */
		public long addWord(String word, String definition, String fuzzy) {
			ContentValues initialValues = new ContentValues();
			initialValues.put(KEY_PLACE, word);
			initialValues.put(KEY_COORDINATES, definition);
			initialValues.put(KEY_FUZZY, fuzzy);
